...
#define EX_NOUSER  -4
#define EX_OK 0
#define DEFAULT_STATHOST "127.0.0.1"
#include "anonymous.h"
#include "buffer.h"
#include "daemon.h"
#include "heap.h"
...
/* 	if (unlink(config.pidpath) < 0) {
	...
	//printf("Config Port:%d",config.port);
	if (child_listen(config.port) < 0) {
		fprintf(stderr, "%s: Could not create listening socket.\n",
			argv[0]);
		exit(EX_OSERR);
	}
/* 	int xi = 0;
	...
	if(xi < 32)
	{
	_beginthread( child_main, 0, NULL );
	}
	xi++;
	} */
	child_main();
